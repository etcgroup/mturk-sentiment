...
function reset_bomb(time) {
    return;
	time = time || 600;
    $('#boxprogress').stop();
    $('#boxprogress').css('background-color', null);
    $('#boxprogress').css('height', '0');
	$('#boxprogress').animate({'height' : '100%'},
							  {'duration' : time,
							   'complete' : explode,
							   'easing' : 'linear'});
...
function youlose() {
    stopit();
    top_score = Math.max(top_score, count);
    menuscreen();
}
function stopit() {
    $('#box').stop();
...
function senddude() {
    ...
    if (dudelaps % 2 == 0)
        newleft = (width/2 - dudewidth/2) + 'px';
    else
        newleft = ('{{=iframe_width}}' - width/2 - dudewidth/2) + 'px';
    

    $('#enemy').animate({'left' : newleft},
						{'duration' : 600,
						 'complete' : function () { dudelaps++; senddude(); },
						 'easing' : 'easeOutExpo'});
}

jQuery(document).ready(menuscreen);
